...
bool Mesh::LoadMesh(const std::string& Filename)
{
	...
	if (m_pScene) {
		...
		m_GlobalInverseTransform = convertAssimpMatrix(m_pScene->mRootNode->mTransformation);
...
void Mesh::LoadBones(uint MeshIndex, const aiMesh* pMesh, std::vector<VertexBoneData>& Bones)
{
	...
	{                
		...
		if (m_BoneMapping.find(BoneName) == m_BoneMapping.end()) 
		{
			...
			m_BoneInfo[BoneIndex].BoneOffset = convertAssimpMatrix(pMesh->mBones[i]->mOffsetMatrix);            
...
void Mesh::ReadNodeHeirarchy(float AnimationTime, const aiNode* pNode, const glm::mat4& ParentTransform)
{
	...
	glm::mat4 NodeTransformation = convertAssimpMatrix(pNode->mTransformation);
